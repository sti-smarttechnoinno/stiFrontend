...
export function VivoPanel({ state, expanded }: { state: PanelState; expanded: boolean }) {
  ...
  const panelT = t?.stiHome?.vivoPanel || {
    title: "Espace VIVO",
    badge: "Production & distribution",
    description: "Smartphones Série V, Série Y & nouveautés.",
    cta: "Explorer l'espace VIVO",
    infoTitle: "Performance & Design",
    infoSub: "Smartphones certifiés VIVO",
    smartphones: "Smartphones",
    news: "Nouveautés",
  }

  const products = [
    { label: panelT.smartphones || "Smartphones", icon: Smartphone },
    { label: panelT.news || "Nouveautés", icon: Sparkles },
  ]
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-warm-gray text-charcoal">
      ...
      <div className="absolute inset-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-warm-gray/75 via-warm-gray/40 to-transparent md:bg-gradient-to-l md:from-warm-gray/75 md:via-warm-gray/40 md:to-warm-gray/10 rtl:md:bg-gradient-to-r" />
      </div>
      ...
      <div className="relative flex h-full flex-col justify-between p-6 sm:p-8 md:p-10 lg:p-14">
        ...
        {/* Info card */}
        <div
          className="absolute bottom-6 end-6 hidden items-center gap-3 rounded-2xl border border-neutral-200/80 bg-white/80 px-4 py-2.5 shadow-xl backdrop-blur-md transition-all duration-[600ms] ease-[cubic-bezier(0.22,1,0.36,1)] sm:bottom-8 sm:end-8 md:flex lg:end-12"
          style={{
        ...
        >
          ...
          <div className="text-start">
            <p className="text-xs font-semibold text-neutral-900 tracking-tight">{panelT.infoTitle}</p>
            <p className="text-[11px] text-neutral-500">{panelT.infoSub}</p>
          </div>
          <Sparkles className="ms-2 h-4 w-4 text-neutral-400 shrink-0" />
        </div>
      </div>
    </div>
  )
